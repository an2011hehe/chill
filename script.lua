...
local function SetupAnimatedBackground(mainFrame)
    ...
    local zoomTween = TweenService:Create(bg, TweenInfo.new(2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, 0, true), {Scale = 1.05})
    zoomTween:Play()
    local moveTween = TweenService:Create(bg, TweenInfo.new(4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, 0, true), {Position = UDim2.new(0.02, 0, 0.02, 0)})
    moveTween:Play()
    return bg, overlay
end
...
SetupAnimatedBackground(MainFrame)
...
UserInputService.InputChanged:Connect(function(input)
    if isDragging and dragStart and startPos and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        local delta = input.Position - dragStart
        local newX = math.clamp(startPos.X.Offset + delta.X, 0, (Camera.ViewportSize.X or 1920) - MainFrame.AbsoluteSize.X)
        local newY = math.clamp(startPos.Y.Offset + delta.Y, 0, (Camera.ViewportSize.Y or 1080) - MainFrame.AbsoluteSize.Y)
...
local function FindNearestTree()
    ...
    for i = 1, #ao do
        ...
        if o:IsA("Model") then
            ...
            for _, kw in ipairs(tk) do
                if string.find(n, kw) then it = true; break end
            end
            if it then
                tp = o:FindFirstChild("HumanoidRootPart") or o:FindFirstChild("Torso") or o:FindFirstChild("PrimaryPart")
                if not tp then
                    local ch = o:GetChildren()
                    for j = 1, #ch do
                        if ch[j]:IsA("BasePart") then tp = ch[j]; break end
                    end
                end
            end
            ...
            if not it then
                if o.Material == Enum.Material.Wood or o.Material == Enum.Material.WoodPlanks then
                    ...
                    if p and not p:FindFirstChildOfClass("Humanoid") then
                        ...
                        for _, kw in ipairs(tk) do
                            if string.find(pn, kw) then it = true; break end
                        end
                    end
                end
            end
        ...
        if it and tp and tp.Parent and not tp.Parent:FindFirstChildOfClass("Humanoid") then
...
local function TeleportToNearestTree()
    safeCall(function()
        ...
        if t then
            local r = getRoot()
            if r then
                r.CFrame = CFrame.new(t.Position + Vector3.new(0, 5, 0))
            end
        end
    end)
end

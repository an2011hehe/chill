...
local function SetupAnimatedBackground(mainFrame)
    ...
    if not isfile(fileName) and writefile then
        pcall(function()
            writefile(fileName, game:HttpGet(imageUrl))
        end)
    end

    local assetPath = isfile(fileName) and getcustomasset(fileName) or imageUrl
    ...
    local zoomTween = TweenService:Create(bg, TweenInfo.new(2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, 0, true), {
        Scale = 1.05
    })
    zoomTween:Play()

    local moveTween = TweenService:Create(bg, TweenInfo.new(4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, 0, true), {

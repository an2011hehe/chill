    ...
    Settings.AimbotConn = Hook(RunService.RenderStepped, function()
        ...
        safeCall(function()
            ...
            if nearestTarget then
                local targetPosition = Camera:WorldToViewportPoint(nearestTarget.Position)
                local smoothness = Settings.AimbotSmoothness
                local smoothingFactor = 1 / (smoothness * 10 + 1)
                
                local currentMouseX = mouseLocation.X
                local currentMouseY = mouseLocation.Y
                local targetMouseX = targetPosition.X
                local targetMouseY = targetPosition.Y
                
                local newMouseX = currentMouseX + (targetMouseX - currentMouseX) * smoothingFactor
                local newMouseY = currentMouseY + (targetMouseY - currentMouseY) * smoothingFactor
...
local function DisableAimbot()
    ...
    if Settings.AimbotConn then
        Settings.AimbotConn:Disconnect()

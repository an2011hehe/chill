...
gifBackground.Parent = Window.UIElements.Main
...
local function playGifBackground(url)
    local success, result = pcall(function()
        local response = game:HttpGet(url)
        local gifData = GifDecoder.Decode(response)
        local frames = gifData.Frames
        if #frames == 0 then
            error("Không có frame")
        end
        for i, frame in ipairs(frames) do
            frame.TextureId = "rbxassetid://" .. frame.AssetId
        end
        return frames
    end)
    if not success then
        warn("[PremiumHub] Lỗi tải GIF:", result)
        pcall(function()
            local response = game:HttpGet(url)
            local gifData = GifDecoder.Decode(response)
            if #gifData.Frames > 0 then
                gifBackground.Image = "rbxassetid://" .. gifData.Frames[1].AssetId
            end
        end)
        return
    end
    ...
    connection = RunService.RenderStepped:Connect(function(deltaTime)
        if not gifBackground.Parent then
            connection:Disconnect()
            return
        end
        elapsedTime = elapsedTime + deltaTime
        if elapsedTime >= frameDelay then
            currentFrame = (currentFrame % totalFrames) + 1
            gifBackground.Image = frames[currentFrame].TextureId
            elapsedTime = 0
        end
    end)
    Settings.GifConnection = connection
end

playGifBackground("https://cdn.discordapp.com/attachments/1233881531404124202/1525688210175430727/C92A5C7B-1B74-4A71-AD62-0075A31245A3.gif?ex=6a63749f&is=6a62231f&hm=e4e28c6a4e9daada020bd5e6da9168a74747dbf274b18a9d2823a2914b29cdcf&")
        ...
        local function setupESP()
            ...
            local renderConnection = Hook(RunService.RenderStepped, function()
                if not Settings.ESPEnabled then
                    box.Visible = false; nameLabel.Visible = false; healthBackground.Visible = false; distanceLabel.Visible = false
                    return
                end
    ...
    Hook(Players.PlayerRemoving, function(player)
        if Settings.ESPBoxes[player] then
            ...
            for _, obj in ipairs(espData) do
                if obj and obj.Destroy and typeof(obj) ~= "RBXScriptConnection" then safeCall(function() obj:Destroy() end) end
            end
...
local function UnloadAll()
    ...
    if Settings.GifConnection then
        Settings.GifConnection:Disconnect()
        Settings.GifConnection = nil
    end

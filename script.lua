...
if mainFrame then
    ...
    videoBg.ZIndex = 0
    videoBg.Parent = mainFrame

    local videoCorner = Instance.new("UICorner")
    videoCorner.CornerRadius = UDim.new(0, 16)
    videoCorner.Parent = videoBg
    ...
    shimmer.Parent = darkOverlay

    task.spawn(function()
        while darkOverlay and darkOverlay.Parent do
...
local function safeCall(func, ...)
    ...
    if not success then
        warn("[PremiumHubPro] Error:", result)
    end
...
local function EnableFly()
    ...
    Hook(UserInputService.InputBegan, function(input, gameProcessedEvent)
        if not gameProcessedEvent then keysDown[input.KeyCode] = true end
    end)
    ...
    Settings.FlyConn = Hook(RunService.RenderStepped, function()
        ...
        if keysDown[Enum.KeyCode.W] then direction = direction + Camera.CFrame.LookVector end
        if keysDown[Enum.KeyCode.S] then direction = direction - Camera.CFrame.LookVector end
        if keysDown[Enum.KeyCode.A] then direction = direction - Camera.CFrame.RightVector end
        if keysDown[Enum.KeyCode.D] then direction = direction + Camera.CFrame.RightVector end
...
local function EnableInfiniteJump()
    ...
    Settings.InfJumpConn = Hook(UserInputService.JumpRequest, function()
        ...
        if currentTime - Settings.InfiniteJumpLastTime < INFINITE_JUMP_THROTTLE then return end
    ...
    Settings.AntiKBConn = Hook(RunService.RenderStepped, function()
        ...
        safeCall(function()
            ...
            for index = 1, #descendants do
                ...
                if instance:IsA("BodyVelocity") and instance ~= Settings.FlyBV then instance:Destroy() end
                if instance:IsA("BodyPosition") then instance:Destroy() end
            end
        end)
    end)
                ...
                if targetPlayer ~= LocalPlayer and targetPlayer.Character then
                    ...
                    if targetRoot and targetHumanoid and targetHumanoid.Health > 0 then
                        ...
                        if onScreen then
                            ...
                            if distanceFromMouse < minimumDistance and distanceFromMouse < 400 then
...
local function EnableESP()
    ...
    local function createESP(targetPlayer)
        ...
        local function setupESP()
            ...
            tracer.BackgroundColor3 = Color3.fromRGB(139, 92, 246)
            ...
            Instance.new("UICorner", healthBackground).CornerRadius = UDim.new(0, 2)
            ...
            Settings.ESPBoxes[targetPlayer] = {box, nameLabel, healthBackground, distanceLabel, renderConnection, tracer, highlight}
        end

        if targetPlayer.Character then setupESP() end
        Hook(targetPlayer.CharacterAdded, function()
            task.wait(0.3)
            if Settings.ESPEnabled then setupESP() end
        end)
    end

    local allPlayers = Players:GetPlayers()
    for index = 1, #allPlayers do createESP(allPlayers[index]) end
    Hook(Players.PlayerAdded, function(player) if Settings.ESPEnabled then createESP(player) end end)
...
local function EnableWallhack()
    ...
    for index = 1, #allObjects do applyWallhack(allObjects[index]) end
    Settings.WallhackConn = Hook(workspace.DescendantAdded, function(object)
        if Settings.Wallhack then applyWallhack(object) end
    end)
...
local function FindNearestTree()
    local currentTime = tick()
    if currentTime - TreeCacheTime < 2 and #TreeCache > 0 then
        local rootPart = getRoot()
        ...
        if nearestTree then return nearestTree end
    end
...
MoveSection:Slider({
    ...
    Callback = function(value) safeCall(function() getHum().WalkSpeed = value end) end,
})
MoveSection:Slider({
    Title = "Sức nhảy",
    Step = 1,
    Value = { Min = 50, Max = 500, Default = 50 },
    Callback = function(value) safeCall(function() getHum().JumpPower = value end) end,
})
MoveSection:Toggle({
...
PlayerSection:Toggle({
    Title = "Nhảy vô hạn",
    Value = false,
    Callback = function(state) if state then EnableInfiniteJump() else DisableInfiniteJump() end end,
})
PlayerSection:Toggle({
    Title = "Anti Knockback",
    Value = false,
    Callback = function(state) if state then EnableAntiKB() else DisableAntiKB() end end,
})
...
AimbotSection:Toggle({
    Title = "Bật Aimbot",
    Value = false,
    Callback = function(state) if state then EnableAimbot() else DisableAimbot() end end,
})
AimbotSection:Slider({
    Title = "Độ mượt",
    Step = 0.05,
    Value = { Min = 0.1, Max = 1.0, Default = 0.5 },
    Callback = function(value) Settings.AimbotSmoothness = value end,
})
...
ESPSection:Toggle({
    Title = "Highlight",
    Value = true,
    Callback = function(state) Settings.ESPHighlight = state end,
})
ESPSection:Toggle({
    Title = "Tracer",
    Value = true,
    Callback = function(state) Settings.ESPTracer = state end,
})
...
WorldSection:Toggle({
    Title = "Wallhack",
    Value = false,
    Callback = function(state) if state then EnableWallhack() else DisableWallhack() end end,
})
WorldSection:Toggle({
    Title = "Fullbright",
    Value = false,
...
WorldSection:Toggle({
    Title = "Đồ họa thấp",
    Value = false,
    Callback = function(state) if state then EnableLowGraphics() else DisableLowGraphics() end end,
})
WorldSection:Toggle({
    Title = "FPS Boost",
    Value = false,
    Callback = function(state) if state then EnableFPSBoost() else DisableFPSBoost() end end,
})
...
Notify("Premium Hub Pro", "Script đã sẵn sàng!", 5)
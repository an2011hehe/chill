...
local function ApplyShadow(obj, size, trans)
    ...
    s.ImageTransparency = trans or 0.65
...
local function SendNotification(title, message, duration, color)
    ...
    ApplyStroke(notif, color or Theme.Accent, 1, 0.2)
    ...
    msgLbl.Size = UDim2.new(1, -20, 0, 14)
    ...
    Tween(notif, {Size = UDim2.new(0, 220, 0, 50), Position = UDim2.new(1, -240, 1, -60)}, 0.3)

    task.delay(duration or 3, function()
        Tween(notif, {BackgroundTransparency = 1, Size = UDim2.new(0, 220, 0, 0)}, 0.4)
        for _, child in ipairs(notif:GetChildren()) do
            if child:IsA("TextLabel") then Tween(child, {TextTransparency = 1}, 0.4) end
        end
...
ApplyShadow(Bubble, 26, 0.55)
ApplyStroke(Bubble, Theme.Accent, 2, 0.4)
ApplyGradient(Bubble, Theme.Accent, Theme.AccentLight, 135)
...
ApplyStroke(BubbleGlow, Theme.Accent, 2, 0.6)
...
ApplyCorner(Ripple, 28)

local function PlayRipple(inputPos)
    local relX = inputPos.X - Bubble.AbsolutePosition.X
    local relY = inputPos.Y - Bubble.AbsolutePosition.Y
    local size = math.max(Bubble.AbsoluteSize.X, Bubble.AbsoluteSize.Y) * 2.5
    Ripple.Size = UDim2.fromOffset(0, 0)
    Ripple.Position = UDim2.fromOffset(relX, relY)
    Ripple.BackgroundTransparency = 0.6
    Tween(Ripple, {Size = UDim2.fromOffset(size, size), BackgroundTransparency = 1}, 0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
end
...
Connect(UserInputService.InputBegan, function(input, gpe)
    ...
    if pos.X >= bPos.X - 12 and pos.X <= bPos.X + bSize.X + 12 and pos.Y >= bPos.Y - 12 and pos.Y <= bPos.Y + bSize.Y + 12 then
        draggingBubble = true
        dragStartPos = input.Position
        bubbleStartPos = Bubble.Position
        Tween(Bubble, {Size = UDim2.new(0, 64, 0, 64), BackgroundTransparency = 0}, 0.1)
        PlayRipple(input.Position)
    end
end)
...
Connect(UserInputService.InputEnded, function(input)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        local sizeX = Bubble.AbsoluteSize.X
        local snapX = SnapToEdge(currentX, sizeX)
        local currentY = math.clamp(Bubble.Position.Y.Offset, 30, screenSize.Y - 70)
        Tween(Bubble, {
            Position = UDim2.new(0, snapX, 0, currentY),
            Size = UDim2.new(0, 56, 0, 56),
            BackgroundTransparency = 0.1
        }, math.clamp(math.abs(currentX - snapX) / 800, 0.15, 0.35), Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
    end
end)
...
ApplyStroke(MainFrame, Theme.Border, 1.5, 0.9)
ApplyShadow(MainFrame, 40, 0.7)
...
local function CloseMenu()
    ...
    task.delay(0.22, function() if not MenuOpen then MainFrame.Visible = false end end)
end
...
TitleBarFix.BackgroundColor3 = Theme.Card
...
ApplyGradient(TitleAccent, Theme.Accent, Theme.AccentLight)
...
VersionLabel.Position = UDim2.new(0, 230, 0, 0)
...
FPSLabel.Position = UDim2.new(0, 290, 0, 0)
...
Connect(RunService.RenderStepped, function()
    ...
    if tick() - lastFPSUpdate >= 0.5 then
        ...
        FPSLabel.TextColor3 = currentFPS >= 50 and Theme.Success or (currentFPS >= 30 and Theme.Warning or Theme.Danger)
        frameCount = 0
        lastFPSUpdate = tick()
    end
end)
...
Sidebar.Size = UDim2.new(0, isMobile and 145 or 160, 1, -48)
...
ContentArea.Size = UDim2.new(1, -(isMobile and 145 or 160), 1, -48)
ContentArea.Position = UDim2.new(0, isMobile and 145 or 160, 0, 48)
...
local function SelectTab(name)
    ...
    for n, d in pairs(TabButtons) do
        local sel = (n == name)
        Tween(d.Btn, {BackgroundTransparency = sel and 0.72 or 1, BackgroundColor3 = sel and Theme.Accent or Color3.fromRGB(255, 255, 255)}, 0.2)
        d.Label.TextColor3 = sel and Theme.Text or Theme.SubText
        d.Icon.TextColor3 = sel and Theme.Text or Theme.SubText
    end
end
...
for _, tab in ipairs(Tabs) do
    ...
    TabButtons[tab.Name] = {Btn = btn, Label = label, Icon = icon}
end
...
local function CreateSection(parent, title)
    ...
    sec.Parent = parent
    ApplyCorner(sec, 12)
    ApplyStroke(sec, Theme.Border, 1, 0.85)
...
local function AddButton(parent, text, callback, color)
    ...
    btn.Parent = parent
    ApplyCorner(btn, 10)
    ApplyStroke(btn, color or Theme.Accent, 1, 0.3)
    ...
    btn.MouseButton1Down:Connect(function() Tween(btn, {Size = UDim2.new(1, 0, 0, 33)}, 0.05) end)
    btn.MouseButton1Up:Connect(function() Tween(btn, {Size = UDim2.new(1, 0, 0, 36)}, 0.05) end)
...
local function AddToggle(parent, text, default, callback)
    ...
    bg.BackgroundColor3 = Theme.Card2
    bg.BorderSizePixel = 0
    bg.Parent = frame
    ApplyCorner(bg, 11)
    ApplyStroke(bg, Theme.Border, 1, 0.8)
    ...
    local function update()
        if state then
            Tween(dot, {Position = UDim2.new(1, -19, 0.5, -8), BackgroundColor3 = Color3.fromRGB(255, 255, 255)}, 0.15)
            Tween(bg, {BackgroundColor3 = Theme.Accent}, 0.15)
            for _, s in ipairs(bg:GetChildren()) do if s:IsA("UIStroke") then s.Color = Theme.Accent end end
        else
            Tween(dot, {Position = UDim2.new(0, 3, 0.5, -8), BackgroundColor3 = Theme.SubText}, 0.15)
            Tween(bg, {BackgroundColor3 = Theme.Card2}, 0.15)
            for _, s in ipairs(bg:GetChildren()) do if s:IsA("UIStroke") then s.Color = Theme.Border end end
        end
    end
    update()
...
local function AddSlider(parent, text, min, max, default, float, callback)
    ...
    sframe.Position = UDim2.new(0, 0, 0, 20)
    ...
    sliderBg.BackgroundColor3 = Theme.Card2
    ...
    ApplyCorner(fill, 3)
    ApplyGradient(fill, Theme.Accent, Theme.AccentLight, 0)
...
local function EnableESP()
    ...
    local function createESP(player)
        ...
        local root = char:FindFirstChild("HumanoidRootPart") or char:WaitForChild("HumanoidRootPart") or char:FindFirstChild("Head")
        ...
        ApplyCorner(box, 3)
        ApplyStroke(box, Theme.Accent, 1.5, 0)
        ...
        healthBg.BackgroundColor3 = Theme.Card
        ...
        local conn = Connect(RunService.RenderStepped, function()
            ...
            if onScreen then
                ...
                if hum then
                    ...
                    healthFill.BackgroundColor3 = hp > 0.6 and Theme.Success or (hp > 0.3 and Theme.Warning or Theme.Danger)
                end
                ...
                if lr then distLabel.Text = string.format("%.0f m", (lr.Position - root.Position).Magnitude) end
                ...
                local color = isEnemy and Theme.Danger or Theme.Accent
    ...
    table.insert(Connections, Players.PlayerRemoving:Connect(function(p)
        if Settings.ESPBoxes[p] then
            for _, obj in ipairs(Settings.ESPBoxes[p]) do safeCall(function() obj:Destroy() end) end
...
local function DisableESP()
    ...
    for _, data in pairs(Settings.ESPBoxes) do
        for _, obj in ipairs(data) do safeCall(function() obj:Destroy() end) end
    end
    Settings.ESPBoxes = {}
end
                ...
                if p ~= LocalPlayer and p.Character then
                    ...
                    if tr and th and th.Health > 0 then
                        ...
                        if dist < Settings.AttackRange then
                            root.CFrame = CFrame.lookAt(root.Position, Vector3.new(tr.Position.X, root.Position.Y, tr.Position.Z))
                            if dist < 10 then
...
local function EnableFPSBoost()
    ...
    SendNotification("FPS Boost", "Đã bật FPS Boost", 2, Theme.Success)
end
...
local function DisableFPSBoost()
    ...
    SendNotification("FPS Boost", "Đã tắt FPS Boost", 2, Theme.Warning)
end
...
local function EnableLowGraphics()
    ...
    for _, v in ipairs(workspace:GetDescendants()) do
        ...
        if v:IsA("Texture") or v:IsA("Decal") then safeCall(function() v:Destroy() end) end
    end
    SendNotification("Đồ họa", "Đã bật đồ họa thấp", 2, Theme.Success)
end
...
local function DisableLowGraphics()
    ...
    SendNotification("Đồ họa", "Đã khôi phục đồ họa", 2, Theme.Warning)
end
...
local function SavePosition(slot)
    ...
    safeCall(function()
        ...
        SendNotification("Vị trí", "Đã lưu vị trí Slot " .. slot, 2, Theme.Success)
    end)
end
...
local function LoadPosition(slot)
    ...
    safeCall(function()
        ...
        if Settings.SavedPositions[slot] then
            ...
            SendNotification("Vị trí", "Đã dịch chuyển đến Slot " .. slot, 2, Theme.Success)
        end
    end)
end
    ...
    safeCall(function()
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character then
                ...
                if tr then
                    local dist = (root.Position - tr.Position).Magnitude
                    if dist < minDist then minDist = dist; nearest = tr end
                end
            end
        end
        if nearest then
            root.CFrame = nearest.CFrame + Vector3.new(0, 3, 0)
            SendNotification("Dịch chuyển", "Đã dịch chuyển đến người chơi gần nhất", 2, Theme.Success)
        end
    end)
end
...
do
    ...
    AddToggle(c, "Bật Bay", false, function(v) if v then EnableFly() else DisableFly() end end)
    ...
    AddToggle(c, "Xuyên tường (NoClip)", false, function(v) if v then EnableNoClip() else DisableNoClip() end end)
end
...
do
    ...
    AddToggle(c, "Không bị đẩy lùi", false, function(v) if v then EnableAntiKB() else DisableAntiKB() end end)

    CreateSection(c, "🎭 NGOẠI HÌNH")
    AddSlider(c, "FOV (Trường nhìn)", 30, 120, 70, false, function(v) Camera.FieldOfView = v end)
end

do
    local c = TabContainers["Visual"]
    CreateSection(c, "👁️ ESP NGƯỜI CHƠI")
    AddToggle(c, "Bật ESP", false, function(v) if v then EnableESP() else DisableESP() end end)
    ...
    AddToggle(c, "Đồ họa thấp", false, function(v) if v then EnableLowGraphics() else DisableLowGraphics() end end)
    AddToggle(c, "FPS Boost", false, function(v) if v then EnableFPSBoost() else DisableFPSBoost() end end)
end

do
    local c = TabContainers["Teleport"]
    CreateSection(c, "📌 LƯU VỊ TRÍ")
    AddButton(c, "Lưu vị trí (Slot 1)", function() SavePosition(1) end, Theme.Accent)
    AddButton(c, "Dịch chuyển về Slot 1", function() LoadPosition(1) end, Theme.Success)
    AddButton(c, "Lưu vị trí (Slot 2)", function() SavePosition(2) end, Theme.Accent)
    AddButton(c, "Dịch chuyển về Slot 2", function() LoadPosition(2) end, Theme.Success)

    CreateSection(c, "🌀 DỊCH CHUYỂN NHANH")
    AddButton(c, "Bay đến người chơi gần nhất", TeleportToNearestPlayer, Theme.Warning)
end

do
    local c = TabContainers["Combat"]
    CreateSection(c, "⚔️ CHIẾN ĐẤU")
    AddToggle(c, "Tự động tấn công", false, function(v) if v then EnableAutoAttack() else DisableAutoAttack() end end)
...
do
    ...
    AddButton(c, "Dọn dẹp workspace", function()
        ...
        SendNotification("Dọn dẹp", "Đã xóa " .. count .. " object", 2, Theme.Success)
    end, Theme.Danger)
    AddButton(c, "Reset nhân vật", function()
        safeCall(function() LocalPlayer.Character:BreakJoints() end)
        SendNotification("Reset", "Đã reset nhân vật", 2, Theme.Warning)
    end, Theme.Warning)
end
...
do
    ...
    AddButton(c, "Hủy giao diện (Unload)", function()
        Settings.ESPEnabled = false; Settings.FlyEnabled = false; Settings.NoClipEnabled = false
        Settings.AutoAttack = false; Settings.InfiniteJump = false; Settings.AntiKnockback = false
        Settings.FPSBoost = false; Settings.LowGraphics = false; Settings.Wallhack = false; Settings.Fullbright = false
        DisableFly(); DisableNoClip(); DisableESP(); DisableAutoAttack(); DisableAntiKB()
...
SelectTab("Home")
SendNotification("Premium Hub", "v3.0 đã sẵn sàng | Chạm ✦ để mở Hub", 5, Theme.Accent)